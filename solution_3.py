...
In each cluster,label propagation algorithm is used to label the unlabel data.
...
#     """
#     pd.read_csv返回的是DataFrame格式，sep：如果不指定参数，则会尝试使用逗号分隔
#     header ：指定行数用来作为列名，数据开始行数。如果文件中没有列名，则默认为0[第一行数据]，否则设置为None。
#     """
#     df = pd.read_csv('semi-bank.csv', header=0, sep=',')  # 现在df是一个DataFrame文件
#     # df = pd.read_csv('../dataset/bank.csv', header=0, sep=';')  # 现在df是一个DataFrame文件
#     # df = pd.read_csv('./dataset/waveform1.dataset', header=None, sep=',')
#     # df = pd.read_csv('./dataset/default_of_credit_card_clients.csv', skiprows=1, header=0)
#     # df = df.drop(df.columns[0], axis=1) #去掉第一列的ID
#     # df = df.sample(frac=1).reset_index(drop=True)  # shuffle dataset rows 清洗数据行
#
...
#     rows = df.shape[0]
#
#     # change month string to int 将月份字符串改为int整型
...
#     # month_str_to_int(df)
#
#     # convert df to dataset examples
#     n_training = 3000
#     """
#     array=[[39 'blue-collar' 'married' 'unknown' 'no' 157 'yes' 'no' 'unknown' 30 5
#       99 6 -1 0 'unknown' 'no']
#      [32 'management' 'single' 'tertiary' 'no' 8436 'yes' 'no' 'cellular' 17
#       4 245 1 149 2 'failure' 'no']...] 以数组获得样本的属性值,类型为numpy.ndarray(n维数组)
#
#     """
#     array = df.head(n_training).values  # head(n)表示取前n行数据
#     set1 = array[:1000, :]  # 获得前1000行数据
...
#     df_test=pd.read_csv('bank-test.csv',header=None,sep=',')
#     n_test = 1521
#     test_set = df_test.values  # DataFrame.tail([n])返回最后的n行的值
#     x_test = test_set[:, :-1]  # 除去最后一列的测试样本
#     y_test = test_set[:, -1]  # 单独的最后一列，样本的真实类别
#
...
#
#     tree = Vfdt(features, delta=0.001, nmin=200, tau=0.5)  # 实例化一个Vfdt对象
