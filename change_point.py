...
class Change_point():
    ...
    def detect_change(self):
        ...
        for k in range(start, end + 1):
            if self.calculateMean(0, k-1) <= 0.5*self.calculateMean(k, N-1): #original 0.9
                skn =0
                scalePreChange = self.calParetoDistScale(0, k-1) #估计变更前的比例参数，在前k个数中找到最小的gamma
                shapePreChange = self.calculateParetoDistShape(scalePreChange, 0, k-1) #估计变更前的形状参数

                scalePostChange = self.calParetoDistScale(k, N-1) #估计变更后的比例参数
                shapePostChange = self.calculateParetoDistShape(scalePostChange, k, N-1) #估计变更后的形状参数
    ...
    def num_cluster(self, rho_multi_delta, n=30): #type(rho_multi_delta)为list
...
#     df = df.values[0]
#     rhodelta = list(df)
#     rhodelta = rhodelta[:20] #取前20个gamma最大的值
#     rhodelta.reverse()
#     print(len(rhodelta))
#     value = []
...
if __name__ == '__main__':
    # path = 'D:/datasets/Sea/norma/Sea-rd-nor-10.csv'
    # changePoint = main(path)
    # print('changePointIndex',changePoint)
    list_ = [1,0.9408793,0.81069136,0.7185438,0.52959245,0.5113219,0.4837961,0.47286355,0.4712481,0.45249987,0.44741607,0.44439897,0.43029112,0.4233997,0.40283474,0.3919778,0.38501608,0.38444453,0.38282862,0.37915555,0.36572105,0.3631272,0.3606204,0.35803863,0.3565097,0.3546012,0.35113776,0.35077932,0.3504002,0.34896547]
    list_2 =[1,0.99842525,0.34642074,0.28553417,0.20135657,0.15913044,0.14404193,0.14271843,0.13992552,0.13751327,0.13490897,0.13308355,0.13165511,0.12969053,0.1280579,0.12783092,0.12643377,0.12083419,0.120462835,0.11915514,0.11665378,0.11570541,0.11550522,0.11536463,0.11522858,0.115130626,0.112640016,0.11242511,0.11167092,0.111236475]
